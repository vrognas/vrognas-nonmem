...
- **chore: publish prep — repo metadata, untrack bundled webview asset, README install section (v0.0.240).** First-publish prep now that `OPEN_VSX_TOKEN` / `VSCE_PAT` are configured. Added `repository` / `bugs` / `homepage` to `package.json` so the Open VSX + Marketplace listing links back to the repo (previously absent — `vsce package` only warned). Dropped `media/lineage/client.js` (+ dev-only `.map`) from git tracking and gitignored both: it's an esbuild bundle regenerated at package time via `vscode:prepublish`, so committing it produced churn (`git rm --cached`; file stays on disk). README gained an **Install** section (Open VSX primary — Positron's default gallery — Marketplace secondary). `.vscodeignore` hardened — vsce ignores `.gitignore` once a `.vscodeignore` exists, so the vsix was shipping `.claude/settings.local.json`, a `.tmp-*.mod` probe scratch file, `.github/**` CI workflows, and stale dev-build `**/*.map` source maps (~170 KB / 65% of the package). Now excludes all of these plus `.env*` / `coverage/` / `*.log` / run-mirror dirs to mirror the sensitive `.gitignore` entries. `private: true` kept (blocks accidental `npm publish`; vsce ignores it). No runtime/behaviour change.

- **ci: add GitHub Actions CI + Release workflows (v0.0.239).** Two workflows added under `.github/workflows/`. `ci.yml` runs on push to `main` and on PRs targeting `main`: Node 22 on Ubuntu, `npm ci` then `lint` / `style-check` / `compile` (tsc --noEmit) / `test` / `package` as separate steps so failures are attributable, plus a 7-day VSIX artifact upload. `release.yml` triggers on `v[0-9]+.[0-9]+.[0-9]+` tag pushes (and `workflow_dispatch` with an explicit tag input). It re-runs the full check suite, then `HaaLeo/publish-vscode-extension@v2` against Open VSX (`OPEN_VSX_TOKEN`) and the VS Code Marketplace (`VSCE_PAT`), and `softprops/action-gh-release@v3` for the GitHub Release with the matching CHANGELOG bullet attached as release notes. Modeled on the `vrognas-nmtran` CI shape and `vrognas-redmyne` publish shape; xvfb dropped (vitest doesn't need a display). Secrets `OPEN_VSX_TOKEN` and `VSCE_PAT` must be configured at the repo level before the first tag push.

- **style: prettier + eslint --fix sweep (v0.0.238).** `npm run lint:fix` cleared 5 pre-existing `curly` violations (`extension.ts:392`, `lineage-panel.ts:176-184`, `parse-lst.ts:534`, `parse-sumo.ts:134`, `lst-decoration-provider.ts:229,231` — added braces around multi-line `if` bodies). `npm run style-fix` reformatted 40 test files to current Prettier policy (mostly multi-lining long expression arguments and switching `"` → `'` in one outlier). `test/views/transforms.test.ts` got a `// prettier-ignore` directive so the `@ts-expect-error` line above the `transforms.js` import doesn't get unpinned by Prettier multi-lining the import block (TS would then attribute TS7016 to the `} from '...js'` line instead of the suppressed line). Pre-requisite for landing CI.

- **chore: enable typed lint rules `no-floating-promises` + `no-misused-promises` (v0.0.237).** `eslint.config.mjs` switched to typed linting (`projectService: true`, `tsconfigRootDir`); the two `@typescript-eslint` rules now error. Caught two `setTimeout(tick, ms)` sites in `src/runtime/run-progress.ts` where the callback returned a Promise (re-arm in the poller's `tick()` and the initial scheduling) — wrapped both in `() => void tick()` so rejections can't escape `setTimeout`'s sync contract. `noUncheckedIndexedAccess` deferred (344 tsc errors across parsers — a separate cleanup).

- **chore: rename followup — output channel "NONMEM", log prefix `[nonmem]`, docs, tests, `.gitignore` (v0.0.236).** Output channel renamed from "Positron NONMEM" to "NONMEM"; log prefixes `[positron-nonmem]` / `[positron-nonmem][vars]` / `[positron-nonmem][session]` → `[nonmem]` / `[nonmem][vars]` / `[nonmem][session]` (12 sites in `extension.ts`); user-facing error string in `positron-api.ts` and code comments in `nmtran-client.ts` / `active-runs-watcher.ts` updated to refer to "the NONMEM extension" / `vrognas.nonmem`. README, CLAUDE.md, AGENTS.md retitled to `vrognas-nonmem`; CLAUDE.md per-run host subdir and workspace-mirror paths reference `vrognas-nonmem` (future-intent — the host runs root and workspace mirror feature is not yet implemented). `.gitignore` now ignores both `.vrognas-nonmem/` (new) and `.positron-nonmem/` (legacy). Test fixtures (`scrub.test.ts`, `signal-dispatch.test.ts`) updated. Historical CHANGELOG entries and `~/positron-nonmem/probe-*` comments in source/tests left alone — they reference real past probe directories on the NONMEM host. `package-lock.json` regenerated.

- **chore: rename extension to `vrognas.nonmem`; command/view namespace → `nonmem.*` (v0.0.235).** Published ID changes from `vrognas.positron-nonmem` to `vrognas.nonmem`; displayName "Positron NONMEM" → "NONMEM"; npm `name` `positron-nonmem` → `nonmem`. Command titles drop the "Positron " prefix (now "NONMEM: Run Current Model" etc.). All `positronNonmem.*` command IDs, view IDs, contextValues, and webview viewTypes → `nonmem.*` (~30 mirrors in `package.json` contributes + runtime contextValue construction in tree providers + `PANEL_VIEW_TYPE` / `FitInspectorProvider.viewType`). Runtime IDs: `positron-nonmem-<tag>` → `nonmem-<tag>`. Stale `positronNonmem.lineages` / `positronNonmem.lineageOverrides` references in code comments corrected to match the actual settings keys (`nonmem.*` — settings namespace was already renamed). **Breaking for existing dev installs:** saved Positron sessions reference the old runtime IDs and will surface "No session manager found for runtime positron-nonmem-primary" errors (per `docs/empirical-notes.md` "Phantom runtime managers") until cleared via *Developer: Clear Workspace State*. User-level extension settings are unchanged (settings root was already `nonmem.*`). Filesystem-visible identifiers (`.positron-nonmem/`, `~/positron-nonmem/`, `positron-nonmem://`) are not actively constructed by current code and stay as-is in this phase; rename to `vrognas-nonmem` when those features get reimplemented.
...
- **feat: decimal-align consistency + 3-decimal padding with dim trailing zeros + payload cleanup (v0.0.232).** Three bundled changes from the 12th-pass review.
  - **User-requested feature: pad to 3 decimals, fade the appended zeros.** `100` displays as `100` + dim `.000`. `1.5` displays as `1.5` + dim `00`. `242.706` unchanged (already 3 decimals). `0` displays as `0` + dim `.000`. Sci values (`4e+05`) unchanged — no mantissa padding. New `fmtNumParts(v)` helper in `formatters.js` returns `{display, fade}`; `decimalAlignSpans` now consumes a raw NUMBER (was a string) and renders the fade in a child `<span class="dim">` so the eye picks up the original precision while the column still aligns via the 4ch frac slot.
  - **`decimalAlignSpans` now applied uniformly in all numeric cell-builders.** Previously `impliedBoundCell` / `impliedInitCell` / the boundary span / `annotatedNumber` rendered numbers as flat text — implicit-bound rows (`-1e+06` / `1e+06`) and explicit-bound rows had their `e` / `.` at different X-positions within the same column. All four now route through `decimalAlignSpans`. `impliedBoundCell` switched from hardcoded display strings (`'-1e+06'`) to numeric sentinels (`-1e6`) so the same fmtNum policy applies.
  - **Dropped 6 unused payload fields.** `InspectorDiagnostics.{ebvShrinkSd, ebvShrinkVr, rseMatrix, seBlockEmitted, hasDesign}` were set but never read by `client.js` (which reads them off `lst` instead). `InspectorDiagnostics.eigenvalues.values` was the full array; only `min`/`max` are surfaced. YAGNI delete. The corresponding `lst.*` fields stay — those ARE used by client.js's `covMethodBadge`.
  - **Stale CSS comments updated** post-v0.0.229's switch from percentage widths to `ch`-unit fixed widths.

- **feat: scientific at |v| ≥ 10000, rounded mantissa, e-as-decimal-anchor (v0.0.231).** Reworked `fmtNum` policy + decimal-align split.
  - **Scientific threshold lowered.** Was `|v| ≥ 1e7` for the 3-decimal mantissa form; now `|v| ≥ 10000` switches to scientific at `toExponential(0)`. Max "normal" value is `9999.999` (8 chars). Examples: `437005.037` → `4e+05`, `1450.234` → `1450.234` (unchanged), `1.807e+35` → `2e+35`, `1000000` → `1e+06`. Full unrounded value lives in `td.title` (`String(v)`) so hover reveals `1.807e+35` even when display shows `2e+35`.
  - **Exponent zero-padded to 2 digits.** `(1e6).toExponential(0)` returns `"1e+6"` but we want `"1e+06"` to match NONMEM's `0.1000E+07` emit format and the existing hardcoded `impliedBoundCell` sentinel display. New `fmtScientific` helper does the padding. Three-digit exponents (`e+262`) pass through unchanged.
  - **Decimal-align split now treats `e` as the anchor.** `decimalAlignSpans` splits at `.` OR `e`/`E` (first match wins). `242.706` → `242|.706`, `2e+05` → `2|e+05` — both have 4ch fractions so the visual anchor sits at the same X. Scientific and decimal rows align on the same vertical line within a column. `min-width: 4ch` on `.num-frac` is now applied to ALL numeric columns (was only IE/FE/Prior).
  - Mirrored the same policy in `src/format-number.ts` so the server-side `formatNumberCompact` matches the WebView's `fmtNum` byte-for-byte.

- **feat: decimal-point alignment + compress huge-magnitude scientific (v0.0.230).** Two related UX bits.
  - **Decimal-point alignment.** Each numeric cell in `rowEl` now emits two child spans: `<span class="num-int">123</span><span class="num-frac">.456</span>`. The fraction span's `min-width: 4ch` (applied per-column to IE / FE / Prior) reserves a fixed-width slot so the leading `.` sits at the same X across rows. Integer-only values (`0`, `1e+06`) get an empty fraction span — still reserves the slot, so the integer's right edge anchors at the decimal-column position. Scientific values with very wide mantissa-and-exponent suffixes (e.g. `.807e+35` = 8ch) push the fraction span past `min-width`; decimal-align is sacrificed for that row but value stays readable.
  - **Compress |exp| ≥ 100 magnitudes.** `2.019e+262` (10 chars) overflowed the 9ch IE column. fmtNum now switches to `toExponential(0)` (`2e+262`, 6 chars) when the absolute exponent reaches 100. Full-precision representation (`String(v)`) is set on the cell's `title` attribute so hover reveals the original `2.019e+262`. Threshold of 1e100 / 1e-99 chosen so reasonably-large pharmacometric values (`1.807e+35`) still get 3-decimal mantissa.

- **feat: fixed `ch` widths on numeric cols, elastic Label (v0.0.229).** Replaced the percentage-based widths (v0.0.227 was 5%/28%/5%/8%/5%/13%/14%/6%/16% etc.) with `ch`-unit fixed widths on every numeric column. Result: only the **Label** column responds to pane-resize — widens when the pane is wider than the sum of numeric widths, ellipsises when narrower. Numeric alignment across THETA / OMEGA / SIGMA tables stays stable regardless of pane width. Widths sized for typical worst-case pharmacometric values: `#`=3ch, `LB`/`UB`=7ch (`-1e+06`), `IE`/`FE`=9ch (`1.807e+35`/`37814.69`), `RSE`=14ch (badge+`%`), `NSD`=5ch, `Shrink`=14ch, `Prior`=10ch, `PriorVar`=16ch. Below the sum-of-numerics (~560px), body scrolls horizontally — bumped `min-width` from 460→560 to reflect this. `ch` scales with the editor font, so users on larger font sizes get proportionally wider columns automatically.

- **fix: clip overflowing numeric cells + explicit alignment per column (v0.0.228).** Two issues addressed in one ship.
  - **Overlap → clip.** User-reported: at narrow side-pane widths, THETA values like `242.706` / `1.807e+35` / `437005.03` overflowed their column allocation and painted OVER adjacent cells (IE bleeds into UB bleeds into P, garbled string `0  242.7061e+06135.639`). OMEGA didn't show the bug because OMEGA's typical values (`0.26`, `0.511`) fit inside their columns. Root cause: numeric `<td>` cells defaulted to `overflow: visible`; only `col-label` carried `overflow: hidden`. With `table-layout: fixed` + `text-align: right`, oversize content painted LEFTWARD past the cell boundary. Fix: `overflow: hidden` on `col-num` / `col-lb` / `col-ie` / `col-ub` / `col-fe` / `col-rse` / `col-nsd` / `col-shrink` / `col-prior` / `col-prior-var`. Trade-off: at very narrow pane widths the LEADING characters get clipped (right-anchored text); the full value now lives in the cell's `title` attribute (rowEl mirrors `td.textContent → td.title` for non-label / non-em-dash cells) so hover reveals it. Deliberately skipped `text-overflow: ellipsis` — an ellipsis on the start of a number looks like part of the number (`…06`) and is more confusing than a clean clip.
  - **Alignment made explicit per column class.** Previously relied on global `td { text-align: right }` + `td:first-child { text-align: left }` overrides + a single `col-label { text-align: left }` rule. Reorganised: every `.col-*` class in `.param-table` now sets its alignment explicitly — `#` / `Label` → left, everything numeric (`LB` / `IE` / `UB` / `FE` / `RSE` / `NSD` / `Shrink` / `P` / `PV`) → right. Decouples alignment from column ORDER (the `td:first-child` rule would break if `#` ever moved) and makes intent obvious in CSS.
...
- **8th-pass review: 5 HIGH-severity fixes (v0.0.217).** From the structural review of `src/` and `media/`:
  ...
  3. **client.js $EST attr merge** — `synthesizeInvisibleAttrs` for $EST was missing the `existingKeys` parameter that $COV correctly passes. The merge loop clobbered any XML-emitted key the helper also covered. Today safe by accident (NM 7.6.0 emits no overlap) but a future-proofing landmine. Threaded `existingKeys` through and reordered the call site so `merged` is computed before synthesis.
  4. **scrub.ts + webview-shell.ts** — added `/Users/<user>/` regex (macOS) to both scrubbers; previously only `/home/<user>/` (Linux) + `C:\` (Windows) were covered. Pharma compute on macOS is rare but real.
  5. **runtime-metadata.ts** — `idTagFromLabel` is user-controlled via `psn.conf [nm_versions]` label. A label like `nm760-on-myserver.corp.example.com` would embed the hostname in `runtimeId`, which Positron may surface to crash reporters / telemetry. Truncate beyond MAX_IDTAG_LEN (16) and append an 8-char SHA-256 hash of the original label so two long labels sharing a prefix still get distinct ids.

- **fix: PV value/badge overlap at narrow pane widths (v0.0.216).** v0.0.215's `inline-flex` wrapper with `flex: 1 1 auto; min-width: 0` allowed the value-part to shrink below its text width — at narrow pane widths the value overflowed its slot visually and ran into the badge. Replaced with a simpler model: drop the flex wrapper, give the badge `display: inline-block; min-width: 3em; text-align: left` so every badge occupies the same horizontal slot regardless of `(7%)` vs `(42%)`. The whole `value (badge)` flows inline, right-aligned by the cell, with the value naturally sitting to the badge's left. Decimal-column alignment holds across rows because the badge always reserves the same width. `.col-prior-var` 13% → 16% so realistic value widths (e.g. `37814.69`) fit without overflowing into the P column.
...
- **feat: ellipsis-truncate long parameter labels (v0.0.210).** Long Pirana-style `; <label>` comments (e.g. `klebsiella_other vs acinetobacter_psudomonas`) previously overflowed the Label column into LB/IE/UB at narrow side-pane widths, garbling the table. The Label column now truncates with a CSS ellipsis; full label is preserved on hover via the cell's `title` attribute.
  ...
  - `client.js` `rowEl`: when rendering a string cell into a `col-label` td, mirror the full text into `td.title` so hover shows the complete label. Skips empty / em-dash cells (no value in surfacing those).
...
- **test: close 7th-review coverage gaps (v0.0.206).** Six new test files and one bug-prevention refactor of an existing one, +57 tests (442 → 499).
  ...
  - **`test/views/webview-shell.test.ts`** (9 tests): privacy-sensitive `sanitizeWebviewMessage` — `vscode-resource://` strip, Windows path strip, `/home/<user>/` strip, 500-char cap, mixed leak types. Plus `buildWebviewShell` CSP construction + style/script ordering.
  - **`test/views/lineage-paths.test.ts`** (7 tests): `toSettingPath` / `fromSettingPath` round-trip, multi-folder first-wins, legacy absolute pass-through, no-workspace fallback. Mock-spy pattern on `vscode.workspace.asRelativePath` + `workspaceFolders`.
  - **`test/shell.test.ts`** (6 tests): `quote()` POSIX + cmd.exe branches via `vi.spyOn(process, 'platform', 'get')` — locks both quote forms so the v0.0.198 win32-fix doesn't silently regress.
  - **`test/views/inspector-threshold-defaults.test.ts`** (3 tests): asserts `INSPECTOR_THRESHOLD_DEFAULTS` keys + values + that `buildInspectorPayload` thresholds match when no overrides supplied. Locks the single-source-of-truth contract.
  - **`test/views/fit-inspector-cell-builders.test.ts`** (15 tests): NEW dual-export pattern at the bottom of `client.js` (mirrors `transforms.js`) so vitest can import `classifyAttrTier` / `buildEstAttrCell` / `buildCovAttrCell` with stubbed browser globals. Covers the three tier branches, NOABORT/NOHABORT disambiguation, PsN-wrapper file-path annotation, MATRIX=R+SPECIAL warning (with the v0.0.204 `(tip || '') +` undefined-guard).
  - **`test/runtime/active-runs-tracker.test.ts`**: added 4 `evictOldestTerminals` tests (under-cap unchanged, over-cap drops oldest by `finishedAt`, running entries NEVER evicted, `markFailed` triggers eviction too) and a file-level `afterEach(() => vi.useRealTimers())` guard so an inline `vi.useFakeTimers()` block leaking past an assertion failure can't pollute every subsequent test in the file.
  - **`test/__mocks__/vscode.ts`** extended with `workspace.asRelativePath`, `workspace.workspaceFolders`, and `QuickPickItemKind` so the new tests can spy/configure cleanly.
  - **`media/fit-inspector/client.js`** bottom: added `module.exports = { classifyAttrTier, buildEstAttrCell, buildCovAttrCell }` behind the same `typeof module !== 'undefined'` guard used by `transforms.js`. WebView ignores; vitest sees.
...
- **fix: 7th-review blockers — privacy hostname purge + tooltip undefined-prefix + settings descriptions (v0.0.204).** Three blocker items from the 7th-review pass shipped together.
  ...
  **`media/fit-inspector/client.js:1340` — `tip += undefined` in `buildCovAttrCell`**: when `classifyAttrTier` returns `tip=undefined` (no tier hit AND no synth match) AND the MATRIX=R+SPECIAL guard fires, `tip +=` yielded `"undefined WARNING: NM silently ignores SPECIAL…"`. Every other tip-mutation site in `buildEstAttrCell` and `buildCovAttrCell` guards with `(tip || '') +`; this one was missed during the v0.0.201 extraction. Aligned.

  **`package.json` settings descriptions stale post-v0.0.202**: `nonmem.lineages` and `nonmem.lineageOverrides` descriptions still claimed "absolute paths". After the v0.0.202 workspace-relative migration, the settings now store relative paths when inside a workspace folder, absolute otherwise. Descriptions now reflect that — visible in the VS Code Settings UI hover for users hand-editing the keys.
...
- **refactor: 5th-review remaining items — refactors + docstring fix (v0.0.198).** Closes out the 5th-review backlog with 4 items:
  ...
  - **DRY2 — extracted relation-edit actions from `lineage-panel.ts`** (new `src/views/lineage-relation-actions.ts`, 353 LOC). The panel was 749 LOC mixing WebView lifecycle + message routing + 7 right-click action handlers + 2 QuickPick helpers. Action handlers (`setParent`, `createRelation`, `writeOverride`, `addToLineage`, `removeFromLineage`, `writeLineages`, `promoteFromPath`) + the shared `pickRunFromGraph` / `quickPickItemForRun` helpers moved out as standalone functions taking a `RelationActionDeps` snapshot. The panel dispatches in `onMessage` via a fresh `actionDeps()` snapshot so reads of `lastGraph` / `currentLineage` reflect the freshest state. `lineage-panel.ts` is now 451 LOC, ~40% smaller.

  - **P5 — platform-aware `quote()`** (`src/shell.ts`). The single-quote idiom (`'value'` with `'\''` escape) is correct for `/bin/sh -c` but cmd.exe treats single quotes as literals, silently breaking commands. Production deployments run on Linux via Positron Remote SSH and never reach the win32 branch, but a wrong-shell quoting bug is silent and confusing if it ever surfaces locally. `quote()` now branches: POSIX → single quotes; cmd → double quotes with `""` doubling escape. Test fixtures (`promote-estimates.test.ts`, `run-model.test.ts`) updated to import `quote` from shell.ts so assertions work cross-platform.

  - **P6 — parse-sumo docstring fix** (`src/runtime/parse-sumo.ts`). The reviewer flagged a regex/docstring mismatch: docstring showed `Condition number    3.245E+02` (no colon), regex required `Condition number:\s+`. Test fixture confirms real-world sumo emits `Condition number: 420.3` (with colon) — the regex was correct, the docstring sample was wrong. Updated the docstring to show both the status row (`Condition number  [  OK  ]`) and the value row (`Condition number: 420.3`) with a note that the colon distinguishes them.

  - **L4 deferred** — pickRunFromGraph showing ALL workspace runs (vs only the curated lineage's) when in a sub-lineage view. Behavioural choice rather than a bug; safe to default to "show all" so users can build cross-lineage relations. Reviewer's reading was reasonable but the inverse is also defensible. Holding pending explicit design guidance.
...
- **feat: $COV options wire→runtime translation (v0.0.184).** Same pattern as $EST atol but applied to all $COV sentinels. New `resolveCovAttrToRuntime(key, value, lastEst, lstTolerances, methodKind)` in xml-cov-defaults.ts resolves:
  ...
  - `cov_tol='-1'` → resolved `covNrd` from `.lst` trace
  - `cov_siglcov='-1'` → inherits $EST `sigl` (via .lst SIGL line or `lastEst.sigl`)
  - `cov_siglocov='-1'` → inherits $EST `siglo`
  - `cov_knuthsumoff='-1'` → inherits $EST `knuthsumoff`
  - `cov_posdef='-1'` → `'0'` for classical methods / `'3'` for EM methods (method-discriminator derived from `lastEst.estimation_method`)
  - `cov_file='BLANK'` (SIR-active) → inherits $EST `file`
  - `cov_format='BLANK'` (SIR-active) → inherits $EST `format`
  - `cov_ranmethod='BLANK'` (SIR-active) → documented default `'3'` (Bauer's $COV doc)

  Payload pre-computes `xmlCovarianceResolved: Record<key, runtime-value>` so the renderer can substitute display text in one lookup. Wire format moved to tooltip for transparency. Added `cov_rmatrix_print`/`cov_smatrix_print` to BARE_COV baseline (default `'no'`) — these are conditionally emitted when user writes `$COV PRINT=R`/`PRINT=S`. 8 new tests for the resolver.
...
- **feat: three follow-up enhancements to the `.lst` parsing layer (v0.0.179).** All three items shipped in one batch since they share the `.lst`-parsing infrastructure added in v0.0.178: (1) **`.lst`-trace tolerance parser**: new `parse-lst-tolerances.ts` extracts BASE / EST / COV TOLERANCE blocks + SIGL/SIGLO from the .lst trace. Inspector now annotates `atol='0'` (XML wire sentinel) with the runtime ANRD value resolved from the .lst (typically 12). Same for `cov_atol='-1'` etc. — tooltip says "(runtime: 12 — from .lst trace; XML wire '0' is a sentinel)". (2) **EM termination codes fix**: `terminationCodeLabel(code, methodKind)` is now method-aware. EM methods (IMP, IMPMAP, SAEM, ITS, DIRECT, BAYES, NUTS) use the documented NM73+ codes (0/8 = completed, 1/9 = ran out of iters, 2/10 = not tested for convergence, 16/24 = OBJF infinite, 32/40 = all OBJFs zero). Classical methods use textmsgs.f90-mapped FORTRAN error numbers. New `classifyEstimationMethodKind()` helper routes by the XML estimation_method attr. Previously the FOCE-style 0-5 mapping was applied universally — wrong for EM. (3) **Propagation 5th tier**: chained $EST steps now distinguish "explicit on this step" (blue, current) from "propagated from previous step" (new muted-blue-italic tier). Uses `findPropagatedKeys()` which cross-references step N-1's options against step N's user tokens (from .lst echo) — empirical NM 7.6.0 rule: explicit user options propagate forward, AUTO-implicit values reset on AUTO=0, so "value match without user typing" is a strong propagation signal. Alias-aware: METHOD=COND token covers cond_estim, NOABORT covers abort, etc. 8 new tests covering all three.
